...
Печатает каждое полученное уведомление и проверяет HMAC-подпись. Полезен, чтобы
глазами увидеть сквозной путь: POST платежа → обработка → webhook. К самому
сервису отношения не имеет, поэтому написан на голой стандартной библиотеке
без зависимостей.
